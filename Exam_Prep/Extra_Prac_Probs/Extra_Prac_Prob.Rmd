...
So, for example: $(5)_2 = 5 \times 4 \times 3 = 60$. Write a function `falling_factorial()` that takes in two inputs, `n` and `k`, and returns the output $(n)_k$. Do **NOT** use the `factorial()` function when constructing `falling_factorial()`; use loops instead! **Hint:** Take a look at the construction of the `newfactorial()` function from HW05.
